...
@testset "Typed Potts Hamiltonian preserves the graph interface" begin
    potts_h = create_example_potts_hamiltonian(Float64)
    topology = potts_h.topology

    @test potts_h isa AbstractGraph
    @test is_directed(potts_h)
    @test edgetype(potts_h) == LabelledEdge{eltype(potts_h)}
    @test hasproperty(potts_h, :labels)
    @test hasproperty(potts_h, :reverse_label_map)
    @test hasproperty(potts_h, :inner_graph)
    @test potts_h.labels === topology.labels
    @test potts_h.reverse_label_map === topology.reverse_label_map
    @test potts_h.inner_graph === topology.inner_graph

    @test indegree(potts_h) == [length(inneighbors(potts_h, v)) for v in vertices(potts_h)]
    @test outdegree(potts_h) == [length(outneighbors(potts_h, v)) for v in vertices(potts_h)]
    @test degree(potts_h) == indegree(potts_h) + outdegree(potts_h)
    @test all(degree(potts_h, v) == indegree(potts_h, v) + outdegree(potts_h, v) for v in vertices(potts_h))
    @test all(neighbors(potts_h, v) == outneighbors(potts_h, v) for v in vertices(potts_h))
    @test all(has_edge(potts_h, e) for e in edges(potts_h))
    @test adjacency_matrix(potts_h) == adjacency_matrix(topology.inner_graph)
end
