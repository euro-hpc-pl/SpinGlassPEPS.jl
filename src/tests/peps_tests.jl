@testset "PEPS - axiliary functions" begin
    ...
    g = M2graph(M)
    gg = interactions2grid_graph(g, (1,1))
...
@testset "compute single tensor from grid" begin
    # TODO this will be done by the function
    #grid, M = form_a_grid((2,2),(3,3))

    g = M2graph(Mq)
    gg = interactions2grid_graph(g, (2,2))

    β = 1.5
    T1 = compute_single_tensor(gg, 1, β)
    T2 = compute_single_tensor(gg, 2,  β)

    g1 = interactions2grid_graph(g, (1,1))

    t1 = compute_single_tensor(g1, 1, β)
    t2 = compute_single_tensor(g1, 2, β)
    t3 = compute_single_tensor(g1, 3, β)
    t4 = compute_single_tensor(g1, 4, β)
    t5 = compute_single_tensor(g1, 5, β)
    t6 = compute_single_tensor(g1, 6, β)
