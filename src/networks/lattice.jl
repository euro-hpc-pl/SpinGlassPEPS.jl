...
function periodic_lattice(size::NTuple{3,Int})
    mm, nn, tt = size
    m, n = 2 * mm, 2 * nn
    map = super_square_lattice((m, n, tt))
    new_map = Dict{Int,NTuple{2,Int}}()
    for (key, val) ∈ map
        i, j = val
        # Fold each doubled axis independently.  The previous implementation
        # used `m` for the row axis as well, which produced invalid coordinates
        # for rectangular lattices, and discarded the requested cluster size.
        push!(new_map, key => (min(i, m - i + 1), min(j, n - j + 1)))
    end
    new_map
end
