...
@inline Base.length(ψ::QMpsOrMpo) = length(ψ.sites)
@inline Base.size(ψ::QMpsOrMpo) = (length(ψ),)
@inline function Base.size(ψ::QMpsOrMpo, dimension::Integer)
    dimension > 0 || throw(ArgumentError("dimension must be positive"))
    dimension == 1 ? length(ψ) : 1
end
