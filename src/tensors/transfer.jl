...
function move_to_CPU!(ten::CentralTensor)
    ten.e11 = move_to_CPU!(ten.e11)
    ten.e12 = move_to_CPU!(ten.e12)
    ten.e21 = move_to_CPU!(ten.e21)
    ten.e22 = move_to_CPU!(ten.e22)
    ten
end

function move_to_CPU!(ten::DiagonalTensor)
    ten.e1 = move_to_CPU!(ten.e1)
    ten.e2 = move_to_CPU!(ten.e2)
    ten
end

function move_to_CPU!(ten::VirtualTensor)
    ten.con = move_to_CPU!(ten.con)
    ten
end

function move_to_CPU!(ten::SiteTensor)
    ten.loc_exp = move_to_CPU!(ten.loc_exp)
    ten
end

function move_to_CPU!(ten::MpoTensor)
    for i ∈ eachindex(ten.top)
        ten.top[i] = move_to_CPU!(ten.top[i])
    end
    for i ∈ eachindex(ten.bot)
        ten.bot[i] = move_to_CPU!(ten.bot[i])
    end
    ten.ctr = move_to_CPU!(ten.ctr)
    ten
end

function move_to_CPU!(ψ::Union{QMpo{T},QMps{T}}) where {T}
    for k ∈ keys(ψ.tensors)
        ψ[k] = move_to_CPU!(ψ[k])
    end
    ψ.onGPU = false
