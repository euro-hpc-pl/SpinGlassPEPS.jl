...
@inline function _saturating_mul(x::Integer, y::Integer)
    x >= 0 && y >= 0 ||
        throw(ArgumentError("bond dimensions and multipliers must be nonnegative"))
    (x > typemax(Int) || y > typemax(Int)) && return typemax(Int)
    x_int, y_int = Int(x), Int(y)
    iszero(x_int) || y_int <= typemax(Int) ÷ x_int ? x_int * y_int : typemax(Int)
end
...
function zipper(
    ...
    Dtemp = _saturating_mul(Dtemp_multiplier, Dcut)
    out = copy(ϕ)
