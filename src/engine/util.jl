...
"""
function exact_spectrum(potts_hamiltonian::PottsLike)
...
"""
function exact_marginal_probability(ctr::MpsContractor{T}, σ::AbstractVector{Int}) where {T}
    # TODO: Not going to work without PoolOfProjectors
...
It uses the `exact_marginal_probability` function for different branch states generated by `branch_states` and normalizes the probabilities.
"""
function exact_conditional_probability(ctr::MpsContractor{T}, σ::Vector{Int}) where {T}
    ...
    probs = exact_marginal_probability.(Ref(ctr), branch_states(local_basis, [σ]))
    probs ./= sum(probs)
end
