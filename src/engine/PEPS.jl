...
mutable struct PEPSNetwork{T<:AbstractGeometry,S<:AbstractSparsity,R<:Real,PH<:PottsLike} <:
    ...
    function PEPSNetwork{T,S,R}(
        ...
        # Keep the projector registry and derived device caches local to this
        # network. Contractors for different lattice transforms can then run
        # concurrently without clearing or mutating one another's workspace.
        lp = copy(projector_pool(potts_hamiltonian))
        net = new{T,S,R,PH}(
            potts_hamiltonian,
...
function decode_state(
    ...
    nodes =
        potts_h_order ? peps.vertex_map.(first(nodes_search_order_Mps(peps))) :
